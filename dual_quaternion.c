...
{
  double absolute = dual_quaternion_absolute(q);
  if(fabs(1+q.w/absolute)<_DUAL_QUATERNION_EPS*absolute) {
    dual_quaternion r = {0.0, 1.0, 0.0, 0.0};
    return r;
  } else {
    double c = sqrt(absolute/(2+2*q.w/absolute));
    dual_quaternion r = {(1.0+q.w/absolute)*c, q.x*c/absolute, q.y*c/absolute, q.z*c/absolute};
    return r;
  }
}
